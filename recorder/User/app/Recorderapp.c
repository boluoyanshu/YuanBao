...
void recorder(const TCHAR *recfile,WM_HWIN hWin)
{
  ...
  rechead.data.ChunkID=0X61746164;	//"data"
	...
	result=f_open(&file,recfile, FA_CREATE_ALWAYS | FA_WRITE | FA_READ); 
	if(result)
		return;
	else
		result=f_write(&file,&rechead,sizeof(__WaveHeader),&bw);//写入头数据
	Usart_SendString(USART1, "Start Recording...");
	...
	while(1)
	{
		if((RecordFlag==2)|| (Flag_ICON001==0))
		{
			...
			*/
			while(i<sectorsize)
			{
				f_lseek(&file,44+i*BUFSIZE);
				result=f_read(&file,comdata,BUFSIZE,(UINT*)&bw);
				if (result)
				{
					printf("err:%d bw:%d\n",result,bw);
					break;//读出错.	  
				...
				while(j<BUFSIZE)
				{
					Usart_SendByte( USART1, comdata[j]);
					j+=1;
				}
				j=0;
				i+=1;
		...
		if((regval>=256)&&(regval<896))
		{
			...
			{	 
				...
				//Usart_SendByte( USART1, regval&0XFF);
				comdata[idx]=regval&0XFF;
				//Usart_SendByte( USART1, regval>>8);
...
//	VS_Sine_Test();
	...
	while(Flag_ICON001)
	{
		if(RecordFlag==1)
		{
			...
			Usart_SendString(USART1, "Recording Done");
			//printf("Recording Done\n");
		}
		if((RecordFlag==0)&&(Replay==1))
		{
			//当点击播放按钮时，等待系统返回指令并播放语音
			scanf("%d",&instr);
			printf("instru = %d\n",instr);
			//sprintf((char *)tempstr,"rec%03d.wav",i);
			sprintf((char *)tempstr,"rec%03d.wav",instr);
			tempstr[10]='\0';
			printf("%s\n", tempstr);
			TEXT_SetText(WM_GetDialogItem(hWin, GUI_ID_TEXT0),tempstr);

			sprintf((char *)recfilename,"1:/recorder/rec%03d.wav",instr);
